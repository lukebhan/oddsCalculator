...
print("Please provide your original bet size, odds for the bet and odds for the hedge bet")
...
val = np.around((calculatePayout(int(stake), int(odds)))/(hedgeProb), 2)
print("You need ", val, " for your hedge bet!")
print("You will spend a total of", val, "for a payout of",np.around(calculatePayout(val, hedge), 4), "with a profit of", np.around(calculatePayout(val, hedge)-val-int(stake), 4))
